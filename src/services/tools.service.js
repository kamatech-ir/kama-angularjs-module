...
export default function toolsService($rootScope, $location) {
  let service = {
    ...
    jalaliToDate__old: jalaliToDate__old, // legacy
    monthDiff: monthDiff,
    yearDiff: yearDiff,
  ...
  function jalaliToDate(jalali) {
    const [year, month, day] = jalali.split('/').map((part) => { return parseInt(part) });
    ...
    g = new Date(
      (gY < 0 ? '-' : '+') +
        ...
        '-' +
        ('0' + (g.getUTCMonth(g) + 1)).slice(-2) +
        '-' +
        ('0' + g.getUTCDate(g)).slice(-2)
    );
  ...
  function jalaliToDate__old(jalali) {
    let date;
